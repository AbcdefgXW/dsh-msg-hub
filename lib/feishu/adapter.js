...
import { describeImage, isVisionEnabled } from "../vision.js";
...
/** 下载飞书图片并识别成文字描述（任何失败都返回空字符串）。 */
async function describeFeishuImage(client, image) {
  try {
    ...
    diagLog(`[feishu] 图片已下载 ${buf.length} 字节，开始识别`);
    return await describeImage(buf, { filename: "photo.jpg" });
  } catch (e) {
    ...
    "im.message.receive_v1": async (data) => {
      ...
      if (!text) {
        ...
        if (audio) {
          ...
          if (!image) return;
          if (!isVisionEnabled()) {
            diagLog(`[feishu ${account.appId}] 收到图片，但未配置视觉模型（缺 state/vision.env）已跳过`);
            return;
          }
          const desc = await describeFeishuImage(client, image);
          if (!desc) {
            diagLog(`[feishu ${account.appId}] 图片识别结果为空，已忽略`);
            return;
          }
          // 加前缀标明来源是图片描述，便于模型区分"看到的"与"用户说的"
          text = `[图片内容] ${desc}`;
        }
      }
